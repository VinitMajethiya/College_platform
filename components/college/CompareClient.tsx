...
const metricGroups = [
  {
    title: "Overview",
    rows: [
      { label: "NIRF Ranking", value: (college: any) => (college.nirfRanking ? `#${college.nirfRanking}` : "N/A") },
      { label: "Overall Rating", value: (college: any) => `${college.rating.toFixed(1)} stars` },
      { label: "Established", value: (college: any) => String(college.established) },
      { label: "Campus Area", value: (college: any) => (college.campusAreaAcres ? `${college.campusAreaAcres} acres` : "N/A") }
    ]
  },
  {
    title: "Fees and Placements",
    rows: [
      { label: "Min Annual Fee", value: (college: any) => formatCurrencyINR(college.annualFeesMin), lowerIsBetter: true },
      { label: "Max Annual Fee", value: (college: any) => formatCurrencyINR(college.annualFeesMax) },
      { label: "Average Package", value: (college: any) => (college.avgPackageLPA ? `Rs ${college.avgPackageLPA} LPA` : "N/A"), higherIsBetter: true },
      { label: "Highest Package", value: (college: any) => (college.highestPackageLPA ? `Rs ${college.highestPackageLPA} LPA` : "N/A") },
      { label: "Placement Rate", value: (college: any) => (college.placementPercent ? `${college.placementPercent}%` : "N/A"), higherIsBetter: true }
    ]
  },
  {
    title: "Academics and Contact",
    rows: [
      { label: "Streams", value: (college: any) => Array.from(new Set(college.courses.map((course: any) => course.type))).join(", ") },
      { label: "Courses Offered", value: (college: any) => `${college.courses.length} courses` },
      { label: "Location", value: (college: any) => `${college.city}, ${college.state}` },
...
export function CompareClient() {
  ...
  const ids = (urlIds !== null ? urlIds : storeItems.map((item) => item.id).join(",")).split(",").filter(Boolean);
  ...
  const { data: collegesRes, isLoading } = useColleges(ids.length > 0 ? { ids: ids.join(",") } : { ids: "null" });
  ...
  const { data: searchRes, isLoading: isSearching } = useColleges({ search: debouncedQuery, pageSize: 5 });
  ...
  function syncIds(nextIds: string[]) {
    ...
    router.replace(nextIds.length > 0 ? `/compare?${next.toString()}` : "/compare");
  }
  ...
  function handleAdd(college: any) {
    ...
    add({ id: college.id, slug: college.slug, name: college.name, city: college.city, state: college.state });
    ...
    return (
      <main className="min-h-screen bg-[#f7f9fc] px-4 py-20">
        <div className="mx-auto max-w-md rounded-[8px] border border-slate-200 bg-white p-8 text-center shadow-sm">
          ...
          <h1 className="text-2xl font-semibold tracking-normal text-slate-950">Your compare tray is empty</h1>
    ...
    <main className="min-h-screen bg-[#f7f9fc]">
      <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
        <div className="mb-8 flex flex-col gap-4 rounded-[8px] border border-slate-200 bg-white p-6 shadow-sm sm:flex-row sm:items-end sm:justify-between">
          <div>
            <p className="text-sm font-semibold text-brand-gold">Compare options</p>
            <h1 className="mt-2 text-3xl font-semibold tracking-normal text-slate-950">Compare colleges</h1>
            <p className="mt-2 text-sm leading-6 text-slate-600">
              Compare fees, placements, ranking, courses, and location side by side.
            </p>
          </div>
          <button
            onClick={share}
          ...
          <table className="w-full min-w-[850px] table-fixed border-collapse text-left text-sm">
            <thead>
              <tr className="border-b border-slate-200">
                <th className="w-48 p-4 text-xs font-semibold uppercase tracking-wide text-slate-400">Details</th>
                {selected.map((college) => <CollegeHeader key={college.id} college={college} onRemove={handleRemove} />)}
                {[...Array(3 - selected.length)].map((_, index) => (
                  <th key={index} className="w-64 border-l border-dashed border-slate-200 bg-slate-50/70 p-6 text-center">
                    <div className="mx-auto mb-3 flex h-10 w-10 items-center justify-center rounded-full bg-slate-100 text-slate-400">
                      <Plus className="h-5 w-5" />
                    </div>
                    <p className="text-xs font-semibold text-slate-500">Add college</p>
                  </th>
                ))}
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-100">
              {metricGroups.map((group) => (
                <>
                  <tr key={`${group.title}-heading`} className="border-y border-slate-200 bg-slate-100/85">
                    <td colSpan={4} className="p-3 text-xs font-bold uppercase tracking-wide text-slate-800">
                      {group.title}
                    </td>
                  </tr>
                  {group.rows.map((row) => (
                    <CompareRow key={`${group.title}-${row.label}`} row={row} selected={selected} emptyCount={3 - selected.length} />
                  ))}
                </>
              ))}
            </tbody>
...
function CollegeHeader({ college, onRemove }: { college: any; onRemove: (id: string) => void }) {
      ...
      <div className="space-y-3">
        <div className="relative flex h-16 items-center overflow-hidden rounded-[8px] bg-brand-navy px-4">
          <Image
            ...
            unoptimized
          />
          <span className="relative z-10 truncate pr-4 text-sm font-semibold text-white">{college.name}</span>
          <button
        ...
        <div className="flex items-center justify-between gap-3">
          <Link href={`/colleges/${college.slug}`} className="text-xs font-semibold text-brand-navy hover:text-brand-gold">
            View detail
          </Link>
          <span className="truncate text-[10px] text-slate-400">{college.city}, {college.state}</span>
        </div>
      </div>
    </th>
  );
...
function CompareRow({ row, selected, emptyCount }: { row: any; selected: any[]; emptyCount: number }) {
  ...
  const bestValue = row.lowerIsBetter
    ? Math.min(...numericValues.filter((value): value is number => typeof value === "number"))
    : row.higherIsBetter
      ? Math.max(...numericValues.filter((value): value is number => typeof value === "number"))
      : null;
  ...
  return (
    <tr className="bg-white transition hover:bg-slate-50/70">
      ...
      {selected.map((college, index) => {
        const isBest = bestValue !== null && numericValues[index] === bestValue && bestValue > 0;
        return (
          <td
            key={college.id}
            className={cn(
          ...
          >
            ...
            {isBest && <span className="mt-1 block text-[10px] uppercase tracking-wide text-brand-gold">Strong option</span>}
          </td>
        );
      })}
      {[...Array(emptyCount)].map((_, index) => (
        <td key={index} className="border-l border-dashed border-slate-200 bg-slate-50/50 p-4" />
      ))}
    </tr>
  );
}
...
function AddCollegeSearch({
  ...
  return (
    <div className="mt-8 rounded-[8px] border border-slate-200 bg-white p-6 shadow-sm">
      <h3 className="mb-3 text-sm font-semibold text-slate-950">Add a college to compare</h3>
      ...
      {searchQuery.trim().length > 0 && (
        <div className="mt-4 space-y-2 border-t border-slate-100 pt-4">
          {isSearching ? (
            ...
            <p className="py-2 text-xs text-slate-400">No colleges found matching &quot;{searchQuery}&quot;</p>
          ) : (
            searchResults.map((college) => {
              const isAlreadyAdded = ids.includes(college.id);
              return (
                <div key={college.id} className="flex items-center justify-between gap-3 rounded-[8px] p-2 transition hover:bg-slate-50">
                  <div className="truncate">
                    <h4 className="truncate text-xs font-bold text-slate-950">{college.name}</h4>
                    <p className="text-[10px] text-slate-400">{college.city}, {college.state}</p>
                  </div>
                  <button
